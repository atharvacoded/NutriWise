...
from backend.schemas import (
    ...
    WeeklyProjection, FoodSearchResult, UserAuth
)
from backend.database import authenticate_or_register
...
ss_path = ROOT / "ss"
if ss_path.exists():
    app.mount("/ss", StaticFiles(directory=str(ss_path)), name="ss")
...
async def serve_login():
    html_path = ROOT / "frontend" / "templates" / "login.html"
    if html_path.exists():
        return html_path.read_text(encoding="utf-8")
    return HTMLResponse("<h1>Login API</h1><p>Login frontend not built yet.</p>")

@app.post("/api/auth")
async def auth_user(auth: UserAuth):
    res = authenticate_or_register(auth.email, auth.password)
    if res["status"] == "success":
        return {"status": "success", "message": res["message"], "user": {"email": auth.email, "id": res["user_id"]}}
    else:
